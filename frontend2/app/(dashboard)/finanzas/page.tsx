...
import {
  ...
  Users, UtensilsCrossed, Settings, ChevronDown, ChevronUp, Pencil,
} from "lucide-react";
...
interface Gasto {
  ...
  categoria: string; notas: string | null;
}

interface SueldoCalc {
  user_id: number; name: string; categoria: string | null;
  sueldo_fijo: number; sueldo_por_hora: number; horas_trabajadas: number;
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex gap-4 items-center justify-center flex-wrap">
        <svg width="200" height="200" viewBox="0 0 200 200">
          {segmentos.map((s, i) => (
            <path key={i} d={segmentoPath(s.inicio, s.fin)} fill={s.color} stroke="#fff" strokeWidth="2" />
          ))}
        </svg>
        <div className="flex flex-col gap-1.5">
          {segmentos.map((s, i) => (
            <div key={i} className="flex items-center gap-2 text-xs">
              <div className="size-3 rounded-full flex-shrink-0" style={{ backgroundColor: s.color }} />
              <span className="capitalize">{s.label}</span>
              <span className="text-muted-foreground ml-auto pl-4">
                {Math.round((s.valor / total) * 100)}% · ${s.valor.toLocaleString("es-AR")}
              </span>
            </div>
          ))}
        </div>
...
export default function FinanzasPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      const reservasConPrecio = reservas.filter(r => r.precio_total && r.status !== "cancelled");
      const reservasSinPrecio = reservas.filter(r => !r.precio_total && r.status !== "cancelled");
      ...
      for (const r of reservasConPrecio) {
        ...
        if (!detalleTemp[key]) {
          detalleTemp[key] = {
            numero: r.room_number || r.room_id.toString(), tipo: r.room_type || "—",
            noches: 0, precio: 0, subtotal: 0,
            es_grupo: !!r.group_id, es_single: r.tipo_ocupacion === "single",
            tiene_precio_total: true, canal: r.channel_name || "directo",
          };
        }
      ...
      if (reservasSinPrecio.length > 0) {
        await Promise.all(Array.from({ length: diasMes }, async (_, i) => {
          ...
          for (const hab of ocupadas) {
            ...
            const precioRes = await authFetch(`${API}/precios/consulta?fecha=${d}&tipo=${tipoConsulta}&hotel_id=${HOTEL_ID}`).then(r => r.json());
            ...
            if (precio > 0) {
              ...
              detalleTemp[key].precio = Math.round(detalleTemp[key].subtotal / detalleTemp[key].noches);
            }
          }
        }));
      }
      ...
      Object.entries(fichajesPorDia).forEach(([key, minsTotalesDia]) => {
        ...
        minsTrabajadosPorEmpleado[userId] = (minsTrabajadosPorEmpleado[userId] || 0) + minsTotalesDia;
        ...
        if (sueldo?.tipo_empleado === "fijo" && sueldo?.horas_diarias) {
          ...
          if (extra > 0) minsExtraPorEmpleado[userId] = (minsExtraPorEmpleado[userId] || 0) + extra;
        }
      });

      const getSueldoDelMes = (userId: number) => {
        const arr = sueldosArray.filter((s: any) => s.user_id === userId);
        return arr.find((s: any) => s.mes === mes + 1 && s.anio === anio) ||
          arr.filter((s: any) => s.anio !== null).sort((a: any, b: any) => (b.anio || 0) - (a.anio || 0) || (b.mes || 0) - (a.mes || 0))[0] ||
          arr[0];
      };
      ...
      const calcSueldos: SueldoCalc[] = userIds.map((userId: any) => {
        ...
        let total = 0, horasMostrar = 0;
        if (sueldo.tipo_empleado === "fijo") {
          const minsExtra = minsExtraPorEmpleado[userId] || 0;
          horasMostrar = Math.round((minsExtra / 60) * 10) / 10;
          total = sueldo.horas_extra_modalidad === "acumular"
            ? sueldo.sueldo_fijo
            : sueldo.sueldo_fijo + minsExtra * (sueldo.sueldo_por_hora / 60);
        } else {
        ...
        return {
          user_id: userId, name: sueldo.name, categoria: sueldo.categoria,
          sueldo_fijo: sueldo.sueldo_fijo || 0, sueldo_por_hora: sueldo.sueldo_por_hora || 0,
          horas_trabajadas: horasMostrar, total: Math.round(total),
          tipo_empleado: sueldo.tipo_empleado, horas_extra_modalidad: sueldo.horas_extra_modalidad,
        };
  ...
  async function handleGuardarGasto() {
    if (!form.descripcion || !form.monto || !form.fecha) { toast.error("Completá todos los campos obligatorios"); return; }
    setGuardando(true);
    try {
      await authFetch(`${API}/gastos?hotel_id=${HOTEL_ID}`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fecha: form.fecha, descripcion: form.descripcion, monto: parseFloat(form.monto), categoria: form.categoria, notas: form.notas || null }),
      });
  ...
  gastos.forEach(g => { gastosPorCategoria[g.categoria] = (gastosPorCategoria[g.categoria] || 0) + Number(g.monto); });
...
detalleIngresos.forEach((d: any) => {
  ...
  function toggleCategoria(cat: string, tipo: "reservas" | "cenas") {
    ...
    else setCategoriasCenas(prev => prev.includes(cat) ? prev.filter(c => c !== cat) : [...prev, cat]);
  }
  function toggleEmpleado(id: number, tipo: "reservas" | "cenas") {
    if (tipo === "reservas") setEmpleadosReservas(prev => prev.includes(id) ? prev.filter(e => e !== id) : [...prev, id]);
    else setEmpleadosCenas(prev => prev.includes(id) ? prev.filter(e => e !== id) : [...prev, id]);
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => setModalConfig(true)} className="gap-2">
        ...
        <Card style={{ borderColor: "#22c55e" }}>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => setDetalleOpen(true)}>Ver detalle</Button>
              <TrendingUp className="size-4 text-green-500" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-green-600">${totalIngresosBrutos.toLocaleString("es-AR")}</div>
            <div className="text-xs text-muted-foreground mt-1">
              Reservas: ${ingresosMes.toLocaleString("es-AR")} · Cenas: ${ingresosCenas.toLocaleString("es-AR")}
            </div>
          </CardContent>
        </Card>
          ...
          return (
            <Card style={{ borderColor: positivo ? "#22c55e" : "#ef4444" }}>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                ...
                {positivo ? <TrendingUp className="size-4 text-green-500" /> : <TrendingDown className="size-4 text-destructive" />}
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold" style={{ color: positivo ? "#22c55e" : "#ef4444" }}>
                  {positivo ? "+" : ""}{resultadoNeto.toLocaleString("es-AR")}
                </div>
                <p className="text-xs text-muted-foreground mt-1">{positivo ? "Ganancia del mes" : "Pérdida del mes"}</p>
              </CardContent>
            </Card>
          );
        })()}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Card><CardContent className="pt-6"><GraficoTorta titulo="Ingresos por origen" datos={datosGraficoIngresos} /></CardContent></Card>
        <Card><CardContent className="pt-6"><GraficoTorta titulo="Egresos por categoría" datos={datosGraficoEgresos} /></CardContent></Card>
      </div>
        ...
        <CardHeader className="cursor-pointer" onClick={() => setExpandDetalleReservas(v => !v)}>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-sm font-bold" style={{ color: resultadoReservas >= 0 ? "#22c55e" : "#ef4444" }}>
                Neto: {resultadoReservas >= 0 ? "+" : ""}{resultadoReservas.toLocaleString("es-AR")}
              </div>
              {expandDetalleReservas ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}
            </div>
          </div>
        </CardHeader>
                  ...
                  <TableBody>
                    {detalleIngresos.map((d: any) => (
                      <TableRow key={d.numero}>
                        ...
                        <TableCell className="capitalize text-xs">{d.tipo}</TableCell>
                    ...
                    <TableBody>
                      {gastosDeReservas.map(g => (
                        <TableRow key={g.id}>
                          ...
                          <TableCell><Badge style={{ backgroundColor: CATEGORIA_COLORES[g.categoria] || "#6b7280", color: "#fff" }} className="text-xs capitalize">{g.categoria}</Badge></TableCell>
                          <TableCell className="text-destructive font-semibold">${Number(g.monto).toLocaleString("es-AR")}</TableCell>
                        </TableRow>
                      ))}
                      {sueldosDeReservas.map(s => (
                        <TableRow key={s.user_id}>
                          <TableCell className="text-xs">{s.name}</TableCell>
                          <TableCell><Badge style={{ backgroundColor: "#f97316", color: "#fff" }} className="text-xs">sueldo</Badge></TableCell>
                          <TableCell className="text-destructive font-semibold">${s.total.toLocaleString("es-AR")}</TableCell>
                        </TableRow>
                      ))}
                      <TableRow>
                        <TableCell colSpan={2} className="font-bold text-right">Total egresos</TableCell>
                        <TableCell className="font-bold text-destructive">${totalEgresosReservas.toLocaleString("es-AR")}</TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
                )}
      ...
      {/* Sección Cenas */}
      <Card>
        <CardHeader className="cursor-pointer" onClick={() => setExpandDetalleCenas(v => !v)}>
          <div className="flex items-center justify-between">
            <CardTitle className="text-base flex items-center gap-2">
              <UtensilsCrossed className="size-4" style={{ color: "#a855f7" }} />Sección Cenas
            </CardTitle>
            <div className="flex items-center gap-4">
              <div className="text-sm"><span className="text-muted-foreground">Ingresos: </span><span className="font-semibold" style={{ color: "#a855f7" }}>${ingresosCenas.toLocaleString("es-AR")}</span></div>
              <div className="text-sm"><span className="text-muted-foreground">Egresos: </span><span className="font-semibold text-destructive">${totalEgresosCenas.toLocaleString("es-AR")}</span></div>
              <div className="text-sm font-bold" style={{ color: resultadoCenas >= 0 ? "#22c55e" : "#ef4444" }}>
                Neto: {resultadoCenas >= 0 ? "+" : ""}{resultadoCenas.toLocaleString("es-AR")}
              </div>
              {expandDetalleCenas ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}
            </div>
          </div>
        </CardHeader>
        {expandDetalleCenas && (
          <CardContent>
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                {ingresosCenas === 0 ? (
                  <p className="text-xs text-muted-foreground py-4">No hay cenas registradas este mes</p>
                ) : (
                  <div className="p-3 rounded-lg border" style={{ borderColor: "#a855f7" }}>
                    <p className="text-2xl font-bold" style={{ color: "#a855f7" }}>${ingresosCenas.toLocaleString("es-AR")}</p>
                    <p className="text-xs text-muted-foreground mt-1">Total cenas del mes</p>
                  </div>
                )}
              </div>
              <div>
                ...
                {gastosDeaCenas.length === 0 && sueldosDeCenas.length === 0 ? (
                  ...
                  <Table>
                    ...
                    <TableBody>
                      ...
                      <TableRow>
                        <TableCell colSpan={2} className="font-bold text-right">Total egresos</TableCell>
                        <TableCell className="font-bold text-destructive">${totalEgresosCenas.toLocaleString("es-AR")}</TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
                )}
      ...
      {/* Gastos */}
      <Card>
        <CardHeader className="flex flex-row items-center justify-between cursor-pointer" onClick={() => setExpandGastos(v => !v)}>
          <CardTitle className="text-base">Gastos del mes</CardTitle>
          <div className="flex items-center gap-2">
            <Button size="sm" onClick={e => { e.stopPropagation(); setModalOpen(true) }} className="gap-2">
              <Plus className="size-4" />Agregar gasto
            </Button>
            {expandGastos ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}
          </div>
        </CardHeader>
        {expandGastos && (
          <CardContent>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          <Button variant="ghost" size="sm" onClick={() => {
                            setGastoEditando(g);
                            setFormEditar({ fecha: g.fecha, descripcion: g.descripcion, monto: g.monto.toString(), categoria: g.categoria, notas: g.notas || "" });
      ...
      {/* Sueldos */}
      <Card>
        <CardHeader className="flex flex-row items-center justify-between cursor-pointer" onClick={() => setExpandSueldos(v => !v)}>
          <CardTitle className="text-base">
            Sueldos del mes
            <span className="text-xs font-normal text-muted-foreground ml-2">(horas de {mesAnteriorNombre})</span>
          </CardTitle>
          {expandSueldos ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}
        </CardHeader>
        {expandSueldos && (
          <CardContent>
            {loading ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-6">No hay sueldos configurados.</p>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {sueldos.map(s => (
                    <TableRow key={s.user_id}>
                      ...
                      <TableCell>{s.categoria ? <Badge variant="outline" className="capitalize">{s.categoria}</Badge> : "—"}</TableCell>
                      <TableCell>
                        <Badge style={{ backgroundColor: s.tipo_empleado === "fijo" ? "#3b82f6" : "#f59e0b", color: "#fff" }}>
                          {s.tipo_empleado === "fijo" ? "Fijo" : "Temporal"}
                        </Badge>
                      </TableCell>
                      <TableCell>{s.tipo_empleado === "fijo" ? `$${s.sueldo_fijo.toLocaleString("es-AR")}` : "—"}</TableCell>
                      <TableCell>{s.horas_trabajadas > 0 ? `${Math.floor(s.horas_trabajadas)}h ${Math.round((s.horas_trabajadas % 1) * 60)}m` : "—"}</TableCell>
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="font-bold text-right">Total sueldos</TableCell>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <Select value={form.categoria} onValueChange={v => setForm(f => ({ ...f, categoria: v }))}>
                  ...
                  <SelectContent>{TODAS_CATEGORIAS_GASTO.map(c => <SelectItem key={c} value={c} className="capitalize">{c}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleGuardarGasto} disabled={guardando}>{guardando ? "Guardando..." : "Guardar gasto"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={modalEditarGasto} onOpenChange={setModalEditarGasto}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleEditarGasto} disabled={guardando}>{guardando ? "Guardando..." : "Guardar cambios"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={detalleOpen} onOpenChange={setDetalleOpen}>
        <DialogContent className="max-w-3xl max-h-[80vh] flex flex-col">
          <DialogHeader><DialogTitle>Detalle de ingresos — {MESES[mes]} {anio}</DialogTitle></DialogHeader>
              ...
              <TableBody>
                {detalleIngresos.map((d: any) => (
                  <TableRow key={d.numero}>
                    ...
                    <TableCell>
                      {d.es_grupo ? <Badge style={{ backgroundColor: "#facc15", color: "#1a1a1a" }}>Grupo</Badge>
                        : d.es_single ? <Badge style={{ backgroundColor: "#f59e0b", color: "#fff" }}>Single</Badge>
                ...
                {ingresosCenas > 0 && (
                  <TableRow>
                    <TableCell colSpan={4} className="font-medium" style={{ color: "#a855f7" }}>Cenas de grupos</TableCell>
                    <TableCell><Badge style={{ backgroundColor: "#a855f7", color: "#fff" }}>Cenas</Badge></TableCell>
                    <TableCell className="font-semibold" style={{ color: "#a855f7" }}>${ingresosCenas.toLocaleString("es-AR")}</TableCell>
                  </TableRow>
                )}
                <TableRow>
                  <TableCell colSpan={5} className="font-bold text-right">Total</TableCell>
                  <TableCell className="font-bold text-green-600">${totalIngresosBrutos.toLocaleString("es-AR")}</TableCell>
                </TableRow>
              </TableBody>
            </Table>
          </div>
      ...
      <Dialog open={modalConfig} onOpenChange={setModalConfig}>
        <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
          <DialogHeader><DialogTitle>Configurar secciones</DialogTitle></DialogHeader>
          <div className="overflow-y-auto flex-1 flex flex-col gap-6 py-2">
            <div className="flex flex-col gap-3">
              <h3 className="font-semibold flex items-center gap-2"><DollarSign className="size-4" />Sección Reservas — Categorías de gastos</h3>
              <div className="flex flex-wrap gap-2">
                {TODAS_CATEGORIAS_GASTO.map(c => (
                  <button key={c} onClick={() => toggleCategoria(c, "reservas")}
              ...
              <div className="flex flex-wrap gap-2">
                {sueldos.map(s => (
                  <button key={s.user_id} onClick={() => toggleEmpleado(s.user_id, "reservas")}
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                {sueldos.map(s => (
                  <button key={s.user_id} onClick={() => toggleEmpleado(s.user_id, "cenas")}
