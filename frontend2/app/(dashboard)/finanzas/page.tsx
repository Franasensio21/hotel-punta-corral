...
import {
  ...
  Users, UtensilsCrossed, Settings, ChevronDown, ChevronUp,
} from "lucide-react";
...
const CATEGORIA_COLORES: Record<string, string> = {
  ...
  desayuno: "#f59e0b", cena: "#8b5cf6", obra: "#ef4444",
};
...
export default function FinanzasPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      const usuariosData = Array.isArray(usuariosRaw) ? usuariosRaw : [];
      setTodosEmpleados(usuariosData);

      // ── Ingresos por cenas ──────────────────────────────────────────
      const cenasData = Array.isArray(cenasRaw) ? cenasRaw : [];
      const totalCenas = cenasData.reduce((sum: number, c: any) => sum + Number(c.total), 0);
      setIngresosCenas(totalCenas);
  ...
  const datosGraficoEgresos = Object.entries(gastosPorCategoria)
    .filter(([, v]) => v > 0)
    .map(([k, v]) => ({ label: k, valor: v, color: CATEGORIA_COLORES[k] || "#6b7280" }));

  // ── Ingresos por canal para gráfico ─────────────────────────────────
  ...
  const CANAL_COLORES: Record<string, string> = {
    cenas: "#8b5cf6", grupo: "#facc15", single: "#f59e0b",
    directo: "#22c55e", booking: "#3b82f6",
  };
  const datosGraficoIngresos = Object.entries(ingresosPorCanal)
    .filter(([, v]) => v > 0)
    .map(([k, v]) => ({ label: k, valor: v, color: CANAL_COLORES[k] || "#6b7280" }));

  // ── Cálculos por sección ─────────────────────────────────────────────
  const gastosReservas = gastos.filter(g => categoriasReservas.includes(g.categoria)).reduce((s, g) => s + Number(g.monto), 0);
  const sueldosReservas = sueldos.filter(s => empleadosReservas.includes(s.user_id)).reduce((s, su) => s + su.total, 0);
  const totalEgresosReservas = gastosReservas + sueldosReservas;
  const resultadoReservas = ingresosMes - totalEgresosReservas;

  const gastosCenas = gastos.filter(g => categoriasCenas.includes(g.categoria)).reduce((s, g) => s + Number(g.monto), 0);
  const sueldosCenas = sueldos.filter(s => empleadosCenas.includes(s.user_id)).reduce((s, su) => s + su.total, 0);
  const totalEgresosCenas = gastosCenas + sueldosCenas;
  const resultadoCenas = ingresosCenas - totalEgresosCenas;

  function toggleCategoria(cat: string, tipo: "reservas" | "cenas") {
    if (tipo === "reservas") {
      setCategoriasReservas(prev => prev.includes(cat) ? prev.filter(c => c !== cat) : [...prev, cat]);
    } else {
      setCategoriasCenas(prev => prev.includes(cat) ? prev.filter(c => c !== cat) : [...prev, cat]);
    }
  }

  function toggleEmpleado(id: number, tipo: "reservas" | "cenas") {
    if (tipo === "reservas") {
      setEmpleadosReservas(prev => prev.includes(id) ? prev.filter(e => e !== id) : [...prev, id]);
    } else {
      setEmpleadosCenas(prev => prev.includes(id) ? prev.filter(e => e !== id) : [...prev, id]);
    }
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Card>
          <CardContent className="pt-6">
            <GraficoTorta titulo="Ingresos por origen" datos={datosGraficoIngresos} />
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <GraficoTorta titulo="Egresos por categoría" datos={datosGraficoEgresos} />
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="text-sm">
                ...
                <span className="font-semibold text-green-600">${ingresosMes.toLocaleString("es-AR")}</span>
              </div>
              <div className="text-sm">
                <span className="text-muted-foreground">Egresos: </span>
                <span className="font-semibold text-destructive">${totalEgresosReservas.toLocaleString("es-AR")}</span>
              </div>
          ...
          {(categoriasReservas.length > 0 || empleadosReservas.length > 0) && (
            <div className="flex flex-wrap gap-1 mt-2">
              {categoriasReservas.map(c => <Badge key={c} variant="outline" className="capitalize text-xs">{c}</Badge>)}
              {sueldos.filter(s => empleadosReservas.includes(s.user_id)).map(s => <Badge key={s.user_id} style={{ backgroundColor: "#3b82f6", color: "#fff" }} className="text-xs">{s.name}</Badge>)}
            </div>
          )}
        </CardHeader>
      </Card>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-base flex items-center gap-2">
              <UtensilsCrossed className="size-4" style={{ color: "#8b5cf6" }} />
              Sección Cenas
            </CardTitle>
            <div className="flex items-center gap-4">
              <div className="text-sm">
                <span className="text-muted-foreground">Ingresos: </span>
                <span className="font-semibold" style={{ color: "#8b5cf6" }}>${ingresosCenas.toLocaleString("es-AR")}</span>
              </div>
              <div className="text-sm">
                <span className="text-muted-foreground">Egresos: </span>
                <span className="font-semibold text-destructive">${totalEgresosCenas.toLocaleString("es-AR")}</span>
              </div>
          ...
          {(categoriasCenas.length > 0 || empleadosCenas.length > 0) && (
            <div className="flex flex-wrap gap-1 mt-2">
              {categoriasCenas.map(c => <Badge key={c} variant="outline" className="capitalize text-xs">{c}</Badge>)}
              {sueldos.filter(s => empleadosCenas.includes(s.user_id)).map(s => <Badge key={s.user_id} style={{ backgroundColor: "#8b5cf6", color: "#fff" }} className="text-xs">{s.name}</Badge>)}
            </div>
          )}
        </CardHeader>
      </Card>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <Select value={form.categoria} onValueChange={v => setForm(f => ({ ...f, categoria: v }))}>
                  ...
                  <SelectContent>
                    {TODAS_CATEGORIAS_GASTO.map(c => <SelectItem key={c} value={c} className="capitalize">{c}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
          ...
          <div className="overflow-y-auto flex-1">
            <Table>
              ...
              <TableBody>
                ...
                {ingresosCenas > 0 && (
                  <TableRow>
                    <TableCell colSpan={4} className="font-medium" style={{ color: "#8b5cf6" }}>Cenas de grupos</TableCell>
                    <TableCell><Badge style={{ backgroundColor: "#8b5cf6", color: "#fff" }}>Cenas</Badge></TableCell>
                    <TableCell className="font-semibold" style={{ color: "#8b5cf6" }}>${ingresosCenas.toLocaleString("es-AR")}</TableCell>
                  </TableRow>
                )}
                <TableRow>
