...
export default function SuperAdminPage() {
  ...
  async function fetchHoteles() {
    setLoading(true);
    try {
      const data = await authFetch(`${API}/superadmin/hoteles`).then((r) =>
        r.json(),
      );
  ...
  async function fetchPagos(hotelId: number) {
    ...
    try {
      const data = await authFetch(
        `${API}/superadmin/hoteles/${hotelId}/pagos`,
      ).then((r) => r.json());
      setPagos((prev) => ({
        ...prev,
        [hotelId]: Array.isArray(data) ? data : [],
      }));
    } catch (e) {
      console.error(e);
    } finally {
  ...
  async function handleCrearHotel() {
    ...
    try {
      const res = await authFetch(`${API}/superadmin/hoteles`, {
        ...
        body: JSON.stringify({
          ...formHotel,
          precio_mensual: formHotel.precio_mensual
            ? parseFloat(formHotel.precio_mensual)
            : null,
          sena_inicial: formHotel.sena_inicial
            ? parseFloat(formHotel.sena_inicial)
            : null,
        }),
      });
      if (!res.ok) {
  ...
  const totalMensual = hoteles
    .filter((h) => h.active)
    .reduce((sum, h) => sum + (h.precio_mensual || 0), 0);
  const totalDeuda = hoteles
    .filter((h) => h.active && !h.sena_pagada)
    .reduce((sum, h) => sum + (h.sena_inicial || 0), 0);
          ...
          <Card className="bg-slate-900 border-slate-800">
            <CardContent className="pt-4">
              ...
              <p className="text-3xl font-bold text-white">
                {hoteles.filter((h) => h.active).length}
              </p>
            </CardContent>
          </Card>
          <Card className="bg-slate-900 border-slate-800">
              ...
              <Card key={hotel.id} className="bg-slate-900 border-slate-800">
                <CardContent className="pt-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div
                        className={`size-2 rounded-full ${hotel.active ? "bg-green-400" : "bg-slate-500"}`}
                      />
                      <div>
                        <p className="font-semibold text-white">{hotel.name}</p>
                        <p className="text-xs text-slate-400">
                          {hotel.city} · {hotel.email}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        ...
                        <p className="font-semibold text-green-400">
                          {hotel.precio_mensual
                            ? `$${hotel.precio_mensual.toLocaleString("es-AR")}`
                            : "—"}
                        </p>
                      </div>
                      <div className="text-right">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <p className="text-xs text-yellow-400 font-semibold">
                            {hotel.sena_inicial
                              ? `$${hotel.sena_inicial.toLocaleString("es-AR")}`
                              : "—"}
                          </p>
                        </div>
                      </div>
                      <div className="flex gap-1">
                        <Button
                          ...
                          onClick={() => {
                            ...
                            setFormEditarHotel({
                              ...
                              email: hotel.email,
                              phone: hotel.phone || "",
                              precio_mensual: hotel.precio_mensual || undefined,
                              sena_inicial: hotel.sena_inicial || undefined,
                              sena_pagada: hotel.sena_pagada,
                              fecha_inicio: hotel.fecha_inicio || "",
                              notas_internas: hotel.notas_internas || "",
                            });
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleToggleActivo(hotel)}
                        >
                          <Power
                            className={`size-4 ${hotel.active ? "text-green-400" : "text-slate-500"}`}
                          />
                        </Button>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => toggleExpandir(hotel.id)}
                        >
                          {expandido === hotel.id ? (
                  ...
                  <div className="flex gap-4 mt-3">
                    <span className="text-xs text-slate-400">
                      {hotel.total_usuarios} usuarios
                    </span>
                    <span className="text-xs text-slate-400">
                      {hotel.total_reservas} reservas totales
                    </span>
                    {hotel.fecha_inicio && (
                      <span className="text-xs text-slate-400">
                        Desde{" "}
                        {format(
                          new Date(hotel.fecha_inicio + "T12:00:00"),
                          "dd/MM/yyyy",
                          { locale: es },
                        )}
                      </span>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormHotel((f) => ({ ...f, ciudad: e.target.value }))
                  }
              ...
              <div className="flex flex-col gap-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormHotel((f) => ({ ...f, telefono: e.target.value }))
                  }
                  className="bg-slate-800 border-slate-700"
                />
              </div>
              <div className="col-span-2 flex flex-col gap-1">
                ...
                <Input
                  value={formHotel.nombre_contacto}
                  onChange={(e) =>
                    setFormHotel((f) => ({
                      ...f,
                  ...
                  placeholder="Nombre del dueño/admin"
                  className="bg-slate-800 border-slate-700"
                />
              </div>
              <div className="col-span-2 flex flex-col gap-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormHotel((f) => ({ ...f, email: e.target.value }))
                  }
                  className="bg-slate-800 border-slate-700"
                />
              </div>
              <div className="col-span-2 flex flex-col gap-1">
                ...
                <Input
                  type="password"
                  value={formHotel.password}
                  onChange={(e) =>
                    setFormHotel((f) => ({ ...f, password: e.target.value }))
                  }
                  className="bg-slate-800 border-slate-700"
                />
              </div>
              <div className="flex flex-col gap-1">
                ...
                <Input
                  type="number"
                  value={formHotel.precio_mensual}
                  onChange={(e) =>
                    setFormHotel((f) => ({
                      ...f,
              ...
              <div className="flex flex-col gap-1">
                ...
                <Input
                  type="number"
                  value={formHotel.sena_inicial}
                  onChange={(e) =>
                    setFormHotel((f) => ({
                      ...f,
                      sena_inicial: e.target.value,
                    }))
                  }
                  className="bg-slate-800 border-slate-700"
                />
      ...
      <Dialog open={modalEditarHotel} onOpenChange={setModalEditarHotel}>
        <DialogContent className="max-w-md bg-slate-900 border-slate-700 text-slate-100">
          ...
          <div className="flex flex-col gap-3 py-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2 flex flex-col gap-1">
                <Label>Nombre</Label>
                <Input
                  value={formEditarHotel.name || ""}
                  onChange={(e) =>
                    setFormEditarHotel((f) => ({ ...f, name: e.target.value }))
                  }
                  className="bg-slate-800 border-slate-700"
                />
              </div>
              <div className="flex flex-col gap-1">
                <Label>Ciudad</Label>
                <Input
                  value={formEditarHotel.city || ""}
                  onChange={(e) =>
                    setFormEditarHotel((f) => ({ ...f, city: e.target.value }))
                  }
                  className="bg-slate-800 border-slate-700"
                />
              </div>
              <div className="flex flex-col gap-1">
                <Label>Teléfono</Label>
                <Input
                  value={formEditarHotel.phone || ""}
                  onChange={(e) =>
                    setFormEditarHotel((f) => ({ ...f, phone: e.target.value }))
                  }
                  className="bg-slate-800 border-slate-700"
                />
              </div>
              <div className="col-span-2 flex flex-col gap-1">
                <Label>Email</Label>
                <Input
                  value={formEditarHotel.email || ""}
                  onChange={(e) =>
                    setFormEditarHotel((f) => ({ ...f, email: e.target.value }))
                  }
                  className="bg-slate-800 border-slate-700"
                />
              </div>
              <div className="flex flex-col gap-1">
                ...
                <Input
                  type="number"
                  value={formEditarHotel.precio_mensual || ""}
                  onChange={(e) =>
                    setFormEditarHotel((f) => ({
                      ...f,
                      precio_mensual: parseFloat(e.target.value),
                    }))
                  }
                  className="bg-slate-800 border-slate-700"
                />
              ...
              <div className="flex flex-col gap-1">
                ...
                <Input
                  type="number"
                  value={formEditarHotel.sena_inicial || ""}
                  onChange={(e) =>
                    setFormEditarHotel((f) => ({
                      ...f,
                      sena_inicial: parseFloat(e.target.value),
                    }))
                  }
                  className="bg-slate-800 border-slate-700"
                />
              ...
              <div className="flex flex-col gap-1">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormEditarHotel((f) => ({
                      ...f,
                      sena_pagada: e.target.value === "true",
                    }))
                  }
              ...
              <div className="flex flex-col gap-1">
                ...
                <Input
                  type="date"
                  value={formEditarHotel.fecha_inicio || ""}
                  onChange={(e) =>
                    setFormEditarHotel((f) => ({
                      ...f,
                      fecha_inicio: e.target.value,
              ...
              <div className="col-span-2 flex flex-col gap-1">
                ...
                <Textarea
                  value={formEditarHotel.notas_internas || ""}
                  onChange={(e) =>
                    setFormEditarHotel((f) => ({
                      ...f,
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setModalEditarHotel(false)}
              className="border-slate-600 text-slate-300 hover:text-white hover:bg-slate-700"
            >
              Cancelar
            </Button>
          ...
          <div className="flex flex-col gap-3 py-2">
            <div className="flex flex-col gap-1">
              ...
              <select
                value={formPago.tipo}
                onChange={(e) =>
                  setFormPago((f) => ({ ...f, tipo: e.target.value }))
                }
                className="h-10 rounded-md border border-slate-700 bg-slate-800 px-3 text-sm text-slate-100"
              >
            ...
            <div className="flex flex-col gap-1">
              <Label>Estado</Label>
              <select
                value={formPago.pagado ? "true" : "false"}
                onChange={(e) =>
                  setFormPago((f) => ({
                    ...f,
            ...
            {formPago.pagado && (
              <div className="flex flex-col gap-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormPago((f) => ({ ...f, fecha_pago: e.target.value }))
                  }
                  className="bg-slate-800 border-slate-700"
                />
              </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              <Input
                ...
                onChange={(e) =>
                  setFormPago((f) => ({ ...f, notas: e.target.value }))
                }
                className="bg-slate-800 border-slate-700"
              />
            </div>
      ...
      <Dialog open={modalEditarPago} onOpenChange={setModalEditarPago}>
        <DialogContent className="max-w-sm bg-slate-900 border-slate-700 text-slate-100">
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setModalEditarPago(false)}
              className="border-slate-600 text-slate-300 hover:text-white hover:bg-slate-700"
            >
              Cancelar
            </Button>
